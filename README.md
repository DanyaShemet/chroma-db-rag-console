...
### 0. Необхідні інструменти

- Node.js (v18+)
- Python (v3.10+)
- pip (йде разом з Python)

---

### 0.1. Встановлення Node.js

1. Скачайте Node.js:
   https://nodejs.org

2. Встановіть (залиште все за замовчуванням)
...
2. ⚠️ ВАЖЛИВО: під час встановлення увімкніть:
   - `Add Python to PATH`
...
### 0.3. Встановлення ChromaDB

Chroma використовується як векторна база і встановлюється через Python:
...
### 0.4. Перевірка перед запуском

Перед початком роботи у вас мають працювати всі команди:
...
### ⚠️ Якщо команда не знаходиться після встановлення

Якщо ви встановили інструмент (наприклад, `chroma`, `node`, `python`), але отримуєте помилку типу: `'command' is not recognized as an internal or external command`

1. Закрити термінал
2. Відкрити новий термінал
3. Запустити команду ще раз

Це потрібно, тому що після встановлення змінюється **PATH**, і поточний термінал може не бачити нові команди.

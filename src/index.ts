...
const muted = (value: string): string => pc.dim(value)
const info = (value: string): string => `${pc.cyan('i')} ${value}`
const success = (value: string): string => `${pc.green('OK')} ${value}`
const warning = (value: string): string => `${pc.yellow('!')} ${value}`
const errorText = (value: string): string => `${pc.red('ERR')} ${value}`
...
function printBox(
  title: string,
  body: string,
  borderColor: NonNullable<Parameters<typeof boxen>[1]>['borderColor'],
): void {
  console.log(
    boxen(body, {
      title,
...
function printRagDiagnostics(diagnostics: {
  ...
  diagnostics.matches.forEach((match, index) => {
    ...
    console.log(
      `  ${pc.cyan(String(index + 1))}. ${pc.bold(match.fileName)} ${muted(`chunk=${chunkLabel}, distance=${distanceLabel}`)}`,
    )
...
async function main(): Promise<void> {
  ...
  printBox(
    'Console RAG Demo',
    `${pc.bold('Status')} ${pc.green('ready')}\n${pc.bold('Chroma collection')} ${process.env.CHROMA_COLLECTION || 'rag_demo_console'}`,
    'cyan',
  )
  printHelp()
    ...
    try {
      ...
      if (command === 'chunks') {
        ...
        chunks.forEach((chunk, index) => {
          ...
          console.log(
            `  ${pc.cyan(String(index + 1))}. ${pc.bold(chunk.fileName)} ${muted(`chunk=${chunkLabel}, length=${chunk.length}`)}`,
          )

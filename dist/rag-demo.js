...
import { answerQuestion, answerQuestionDirect, getChatModelName, getEmbedding, getEmbeddingModelName, } from './openai.js';
...
export async function getIndexedChunks(filter) {
    ...
    return records.ids
        ...
        return (chunk.fileName.toLowerCase().includes(normalizedFilter) ||
...
export async function getIndexedChunk(filter, chunkIndex) {
    const normalizedFilter = filter.trim().toLowerCase();
    if (!normalizedFilter) {
        return null;
    }
...
//# sourceMappingURL=rag-demo.js.map